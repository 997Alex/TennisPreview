...
class TennisDailyPipeline:
    ...
    async def _load_historical_data(self, recent_years: int = 6, force: bool = False):
        ...
        # Fresh results (every startup): ESPN completed matches replayed
        # onto profiles/H2H/training so form is always current.
        try:
            recent = self._update_recent_results()
            if recent:
    ...
    async def _get_odds_for_matches(self, matches: List[TennisMatch],
        ...
        if self._has_api_key(self.theoddsapi):
            try:
                toa_odds = self.theoddsapi.get_odds_for_matches(match_ids)
                for mid, odds_list in toa_odds.items():
    ...
    def _select_decisions(self, value_bets: List[ValueBet],
        ...
        def _decorate(bet: ValueBet) -> BetDecision:
            ...
            flag, note = sisal_coverage(bet.tier)
            if note:
